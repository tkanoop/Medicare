    ...
    submit: async (req, res) => {
       ...
       login: async (req, res) => {
        try {
          ...
          if (existUser) {
            ...
            res.status(401).json({err:"You have been blocked from accessing this Website"})
      ...
      getDoctorsByDepartment:async(req,res)=>{
        try {
          ...
          console.log(departments);
          const details=await Doctor.find({$and:[{department:departments.department},{status:true}]})
      ...
      bookDoctor:async(req,res)=>{
        ...
        try {
          ...
          let  booked = await Booking.findOne({ $and: [{ date: date }, { starting_time: time },{doctor_id:id.name}] }) 
